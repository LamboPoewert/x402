...
| `smartWalletMaxComputeUnits` | `number` | `400000` | Maximum compute units allowed for smart wallet transactions. Must be a safe integer ≥ 1. Throws at construction when `enableSmartWalletVerification` is `true` and the value is non-finite or fractional. |
| `smartWalletMaxPriorityFeeMicroLamports` | `number` | `50000` | Maximum priority fee in microlamports for smart wallet transactions. Must be a safe integer ≥ 0. Throws at construction when `enableSmartWalletVerification` is `true` and the value is non-finite or fractional. |
...
To validate smart wallet limit values before constructing `ExactSvmScheme`, use the exported `assertSmartWalletLimits` helper:

```typescript
import { assertSmartWalletLimits } from "@x402/svm";
import type { SmartWalletLimits } from "@x402/svm";

const limits: SmartWalletLimits = {
  maxComputeUnits: parseInt(process.env.MAX_CU ?? "400000"),
  maxPriorityFeeMicroLamports: parseInt(process.env.MAX_FEE ?? "50000"),
};

// Throws if either value is non-finite or fractional
assertSmartWalletLimits(limits);
```

This is useful when reading limits from environment variables, where `parseInt` on an unset variable returns `NaN` and would otherwise silently disable fee protections.
